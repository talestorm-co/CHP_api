...
|[Connect](#Connect)                        | Отправляет запрос с подключением к серверу                         |                       |
|[Reconnect](#Reconnect)                    | |
...
Параметры:
 ...
 - password: str - пароль пользователя в ITI Capital
...
Возвращает в **случае успеха**:
   Рыночные данные по всем подписаным котировкам
...
Параметры:
 - symbols - пустое или имя/список котировок для подписки
   * пустое (не переданы параметры) - отправит запрос на отписку от всех котировок из [listening_quotes](#listening_quotes)
   * list[str] - список нескольких котировок (Пример: ['SBER', 'GAZP'])
   * str - одна котировка для подписки (Пример: 'SBER')
